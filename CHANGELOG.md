...
- iOS/macOS now recover automatically from route changes (headphone unplug, Bluetooth) and, on iOS, from media services resets: the whole engine, samplers and loaded banks are rebuilt from tracked state. The audio session is re-activated before restarting the engine, and `playNote` self-heals a stopped engine as a last resort (thanks @avour, #55).
